...
async def save_processed_agent_data(processed_data: ProcessedAgentData):
    # Add processed data to Redis
    redis_client.lpush("processed_agent_data", processed_data.model_dump_json())
    
    # Check if we've accumulated enough data for a batch
    if redis_client.llen("processed_agent_data") >= BATCH_SIZE:
        # Extract, validate, and accumulate batch data
        batch_data = [
            ProcessedAgentData.model_validate_json(redis_client.lpop("processed_agent_data"))
            for _ in range(BATCH_SIZE)
        ]

        # Save the accumulated batch data
        store_adapter.save_data(processed_agent_data_batch=batch_data)
